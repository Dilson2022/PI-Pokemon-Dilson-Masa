...
const getAllPokemonPost = async (req, res) => {
  try {
    const {id, nombre, tipo, ataque, defensa, velocidad, altura, peso, } = req.body;
    const newPokemon = await createPokemon(id, nombre, tipo, ataque, defensa, velocidad, altura, peso);
